%% TRUNCATED FIRST-DIFFERENCE RULE (TFDR) 
%  Some more text
...
for itau = 2:config.taumax
    ResM(:,vars.i_rule,itau) = ResM(:,vars.i_lag,itau) + param.phi_pi*...
        ResM(1:end,vars.pi,itau) + param.phi_x*ResM(:,vars.x,itau); %without max operator
end
clearvars itau
