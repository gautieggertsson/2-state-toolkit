...
for itau = 2:config.taumax
    ResM(:,vars.ngdp,itau) = ResM(:,vars.x,itau) + [ResM(2:end,vars.p_lag,itau);
        ResM(end,vars.p_lag,itau)];
end
clearvars itau
...
graphing(IR,vars,25,'variables',{'i','pi','x','y_lag','ngdp'},'cont_data',ResM,'cont_num',1:5:200)