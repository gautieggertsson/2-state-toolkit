...
[D_1,G_1, ResM, max_k,k,T_tilde] = regime1(AAA,BBB,D_3a,D_3,D_2,G_3,G_2,param,config,"verbose",0);
toc 
...
for tau=1:config.taumax
    ...
    for t=1:size(ResM,1)-1
        ResM(t,vars.checkR,tau) = param.psi_yc*ResM(t+1,vars.ny_lag,tau);
    end
end
...
graphing(IR,vars,50,["y","R","pi","checkR","dcheckR","c","i","k","ny_lag","p_lag","pstar","pgap","ngdpgap","b","sigma"],ResM,[2:5:50])
