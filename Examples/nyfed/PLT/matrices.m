...
% p target rule
AAA(eq.ptar_rule,vars.ptar_rule)  = -1; 
AAA(eq.ptar_rule,vars.p_lag)      = param.kappa_rule/param.lagr_rule;
AAA(eq.ptar_rule,vars.pstar)      = -param.kappa_rule/param.lagr_rule;
BBB(eq.ptar_rule,vars.y)          = -1;

% Truncated Taylor rule (contemporaneous inflation and output)
AAA(eq.rule,vars.ptar_rule) = 1;
