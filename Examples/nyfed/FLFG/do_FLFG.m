...
%% SPECIFY DURATION OF FORWARD GUIDANCE
k = zeros(1,config.taumax);
if isfield(param,'fg_target') 
else
    param.fg_target = 6;    % fixed length FG length parameter nu
end
if isfield(param,'T_tilde_fg')
else
    param.T_tilde_fg = 1;   
end
              
for t = param.T_tilde_fg+1:param.T_tilde_fg+1+param.fg_target-1
    k(t) = param.T_tilde_fg+param.fg_target - t;
end
clearvars t
...
[D_1,G_1, ResM, max_k,k,T_tilde] = regime1(AAA,BBB,D_3a,D_3,D_2,G_3,G_2,param,config,'k_input',k,'T_tilde_input',param.T_tilde_fg,'R0_search',0);
...
graphing(IR,vars,30','cont_data',ResM,'cont_num',2:30)