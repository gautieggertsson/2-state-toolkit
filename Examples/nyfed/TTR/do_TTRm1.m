...
for tau=1:config.taumax
    for t=1:size(ResM,1)-1
        ResM(t,vars.checkR,tau) = param.psi_pi*ResM(t,vars.pi_lag,tau) + param.psi_y*ResM(t,vars.y_lag,tau);
    end
end
...
graphing(IR,vars,50,'variables',{'y','R','pi','checkR','dcheckR','c','i','k','kbar_lag','b','sigma'},'cont_data',ResM,'cont_num',[2:5:50])
