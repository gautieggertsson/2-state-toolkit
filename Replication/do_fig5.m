%% FIGURE 5, A.1, A.2, A.3. TABLE 2. COMPARE RULES IN THE 3 EQUATION 
%% NEW-KEYNESIAN MODEL UNDER A COST PUSH SHOCK
...
%% SET PARAMETERS
cfg.models  = {'OCP','TTR0','PLT','NGDPT','HDNGDPT','SDTR','TTRP','ATR','SUP','TTRSm1','TTRm1','AIT','FLFG'};
...
graph_models(PanelA,cfg.horizon,var_labA,mod_lab(mod_base),col_base,line_base);

% Baseline table, last 4 vars (Panel B)
graph_models(PanelB,cfg.horizon,var_labB,mod_lab(mod_base),col_base,line_base);
...
graph_models(PanelA,cfg.horizon,var_labA,mod_lab(mod_extra),col_extra,line_extra);

% Other rules table, last 4 vars (Panel B)
graph_models(PanelB,cfg.horizon,var_labB,mod_lab(mod_extra),col_extra,line_extra);
clear PanelA PanelB
