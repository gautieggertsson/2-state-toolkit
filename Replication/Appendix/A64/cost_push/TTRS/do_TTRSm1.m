...
for itau = 2:config.taumax
    ResM(:,vars.i_rule,itau) = param.phi_i*ResM(:,vars.i_lag,itau) + (1-param.phi_i)*(ResM(:,vars.rstar,itau) + param.phi_pi*...
        ResM(:,vars.pi_lag,itau) + param.phi_x*ResM(:,vars.x_lag,itau)); %without max operator
end
clearvars itau
...
graphing(IR,vars,25,'variables',{'i','pi','x','i_lag','i_rule','phi1'},'cont_data',ResM,'cont_num',1:20)